...
export default function NotitiesPage() {
    ...
    const inputStyle = { width: '100%', padding: '10px 12px', border: '1.5px solid #e2e8f0', borderRadius: '10px', fontSize: '0.9rem', color: '#1e293b', boxSizing: 'border-box', fontFamily: 'inhe[...]
};
        ...
        <div style={{ display: 'flex', flexDirection: 'column', background: '#f1f5f9' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #F5850A 0%, #D96800 100%)', padding: '14px 20px', flexShrink: 0, boxShadow: '0 2px 12px rgba(245,133,10,0.3)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '42px', height: '42px', borderRadius: '50%', background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShr[...]
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.87rem' }}>Persoonlijke memo&rsquo;s en checklists</div>
                    </div>
                </div>
            </div>
            <div style={{ padding: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                        <button onClick={() => setModal('memo')} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 12px', background: '#fff8f0', border: '1.5px solid #fde8cc',[...]
                            <i className="fa-solid fa-note-sticky" /> Memo
                        </button>
                        <button onClick={() => setModal('checklist')} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 12px', background: '#f0fdf4', border: '1.5px solid #86e[...]
                    ...
                    {notities.map(n => {
                        ...
                        return (
                            <div key={n.id} style={{ background: kl.bg, border: `1.5px solid ${kl.border}`, borderRadius: '14px', padding: '14px', position: 'relative' }}>
                                {/* Verwijder knop */}
                                <button onClick={() => verwijder(n.id)} style={{ position: 'absolute', top: '10px', right: '10px', background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5[...]
                                    ...
                                    <>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '8px' }}>
                                            {n.items.map((it, i) => (
                                                <div key={i} onClick={() => toggleCheck(n.id, i)}
                                                    ...
                                                    <div style={{ width: '18px', height: '18px', borderRadius: '4px', flexShrink: 0, border: `2px solid ${it.gedaan ? '#10b981' : '#cbd5e1'}`, backgrou[...]
                                                        {it.gedaan && <i className="fa-solid fa-check" style={{ color: '#fff', fontSize: '0.9rem' }} />}
                                                    </div>
                                                    <span style={{ fontSize: '0.85rem', color: it.gedaan ? '#94a3b8' : '#334155', textDecoration: it.gedaan ? 'line-through' : 'none', flex: 1 }}>{it.t[...]
                                                </div>
                                            ))}
                                        </div>
                                        {/* Voortgang */}
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                                            <div style={{ flex: 1, height: '4px', borderRadius: '2px', background: '#e2e8f0', overflow: 'hidden' }}>
                                                <div style={{ height: '100%', width: `${totaal ? (gedaan / totaal) * 100 : 0}%`, background: gedaan === totaal && totaal > 0 ? '#10b981' : '#F5850A', b[...]
                                                </div>
                                            </div>
                                            <span style={{ fontSize: '0.84rem', fontWeight: 700, color: gedaan === totaal && totaal > 0 ? '#10b981' : '#94a3b8', flexShrink: 0 }}>{gedaan}/{totaal}</sp[...]
                                        </div>
                                    </>
                                )}
                ...
                {modal && (
                    <>
                        ...
                        <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '480px', background: '#fff', borderRadius: '20px 20px 0 0', pa[...]
                            ...
                            {modal === 'checklist' && (
                                <div style={{ marginBottom: '10px' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                                        ...
                                        <button onClick={voegItemToe} style={{ padding: '0 14px', background: '#10b981', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 700, fontSize[...]
                                    </div>
                                </div>
                            )}
                            ...
                            <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
                                {KLEUREN.map(k => (
                                    <button key={k.id} onClick={() => setKleur(k.id)} style={{ width: '28px', height: '28px', borderRadius: '50%', background: k.bg, border: `2.5px solid ${kleur === k[...]
                                ))}
