...
const targetDir = path.join(__dirname, 'src', 'app', 'medewerker');
...
files.forEach(file => {
    // Skip planning and page.js in medewerker root since we already handled them or they have custom logic
    if (file.includes('planning\\page.js') || file.includes('planning/page.js')) return;
    if (file === path.join(targetDir, 'page.js')) return;
    if (file === path.join(targetDir, 'layout.js')) return;
