const fs = require('fs');
let content = fs.readFileSync('src/app/profiel/page.js', 'utf8');

// 1. Update zzpSections array
content = content.replace(
    "{ id: 'vca', label: 'VCA Certificaat', icon: 'fa-certificate' },",
    "{ id: 'kvk', label: 'KVK Uittreksel', icon: 'fa-file-invoice' },\n        { id: 'vca', label: 'VCA Certificaat', icon: 'fa-certificate' },"
);

// 2. Remove KVK fields from 'gegevens'
const oldGegevensKvkBlock = `                                <div>
...
content = content.replace(oldGegevensKvkBlock, "");
// Remove the '<div></div>' spacer after BTW Nummer which was there for the grid
content = content.replace(
...
// 3. Insert the new KVK section right before the VCA section
const newKvkSection = `                    {/* ════════ KVK UITTREKSEL (ZZP) ════════ */}
...
content = content.replace("{/* ════════ VCA (ZZP ONLY) ════════ */}", newKvkSection);

// Update section logic in sectionAlert
content = content.replace(
...
fs.writeFileSync('src/app/profiel/page.js', content);`;
